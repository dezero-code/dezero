      ...
      <div class="col-lg-3">
        <div class="panel panel-tree category-tree-wrapper">
          ...
          <div class="panel-body">
            ...
            <div class="dd dd-category-group" id="category-nestable-wrapper" data-name="category" data-max-depth="<?= $category_model->getMaxLevels(); ?>" data-url="<?= Url::to("/category/{$category_model->category_type}/weight"); ?>?category_id=<?= $category_model->category_parent_id; ?>"<?php if ( ! $category_model->config->isEditable() ) : ?> data-readonly="true"<?php endif; ?>>
              <?=
              ...
              ?>
                ...
                <div class="buttons">
                  ...
                  ?>
                    <a href="<?= Url::to("/category/{$category_model->category_type}/create", ['parent_id' => $category_model->category_id]); ?>" class="btn mr-10 mb-10 btn-primary"><i class="icon wb-plus"></i> <?= $category_model->config->text('sub_add_button'); ?></a>
                  <?php
                  ...
                  ?>
                    ...
                    <ul class="dropdown-menu">
                      ...
                      ?>
                        <li><a href="<?= Url::to("/category/{$category_model->category_type}/create", ['parent_id' => $category_parent_model->category_id]); ?>" class="dropdown-item"><i class="icon wb-plus"></i> <?= $category_model->config->text('sub_add_button'); ?> <?= Yii::t('backend', 'of'); ?> <strong><?= $category_parent_model->title() ?></strong></a></li>
                      <?php endforeach; ?>
                      <?php
                        // Current category can have children?
                        if ( $category_model->depth < $category_model->config->getMaxLevels() ) :
                      ?>
                        <li><a href="<?= Url::to("/category/{$category_model->category_type}/create", ['parent_id' => $category_model->category_id]); ?>" class="dropdown-item"><i class="icon wb-plus"></i> <?= $category_model->config->text('sub_add_button'); ?> <?= Yii::t('backend', 'of'); ?> <strong><?= $category_model->title() ?></strong></a></li>
                      <?php endif; ?>
                    </ul>
                  <?php endif; ?>
                </div>
